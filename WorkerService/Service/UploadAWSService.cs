    ...
    class UploadAWSService
    {
        ...
        public DataTable ScanUpload(string connectionString)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
                    //Quét lại file lỗi 3 lần (3,4,5)
                    string query = @"SELECT TOP 1000 ea.ID,ea.URL, ea.Name FROM eAttachment ea join eBook e ON ea.eBook_ID = e.ID 
                    WHERE ea.isUpload in (0,3,4,5) and ea.URL is not null AND e.IsLinkYoutube = 0";
                    connection.Open();
